...
/**
 * Update year vision (simple text field)
 * Non-form mutation with simple signature
 * 
 * @param vision - New year vision text
 * @returns Success response
 */
export async function updateYearVision(vision: string) {
  try {
    const result = await withAuth(async (user) => {
      const userId = user.id;
      const db = getDatabaseClient();
      ...
      return createActionSuccess({ vision });
    })({});
    
    if (result.failed) {
      throw new Error(result.errors._errors?.join(', ') || 'Failed to update year vision');
    }
    ...
    console.error('Failed to update year vision:', error);
    return handleActionError(error, 'Failed to update year vision');
  }
}
